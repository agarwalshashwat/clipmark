...
// Classic content-script twin of extension/src/loop.module.js (ESM exports).
// KEEP IN SYNC: any change here must be mirrored in src/loop.module.js.
//
 ...
 */
function loopWrapEpsilon(playbackRate = 1, tickSeconds = LOOP_DEFAULT_TICK) {
 ...
 */
function normalizeLoopSegment(pair, duration = 0) {
  if (!pair) return null;
 ...
 */
function insertLoopSegment(segments, pair, duration = 0) {
 ...
 */
function updateLoopSegmentBound(segments, index, which, time, duration = 0) {
  const list = Array.isArray(segments) ? segments : [];
...
function removeLoopSegment(segments, index) {
  const list = Array.isArray(segments) ? segments : [];
  if (!Number.isInteger(index) || index < 0 || index >= list.length) return [...list];
 ...
 *          (see loopEditAnchor).
 */
function advanceLoop(state, tick) {
 ...
 */
function loopEditAnchor(currentTime, correction, nowMs, graceSeconds = LOOP_EDIT_GRACE) {
 ...
 */
function needsOverlayRemount(overlayParent, fullscreenElement, body) {
 ...
 */
function shouldRebindVideo(cached, live) {
...
function isLoopBookmark(bookmark) {
  return !!bookmark &&
...
function sanitizeLoopName(name, segment) {
...
function formatLoopClock(seconds) {
 ...
 */
function formatLoopRange(bookmark) {
 ...
 */
function buildLoopBookmark({ videoId, segment, name, videoTitle = null, reviewSchedule = [], nowMs = 0 }) {
 ...
 */
function isDuplicateLoop(bookmarks, segment) {
  const seg = normalizeLoopSegment(segment);
  if (!seg) return false;
...
// `typeof advanceLoop === 'function'` guards. Same pattern as src/recall.js.
globalThis.loopWrapEpsilon = loopWrapEpsilon;
globalThis.normalizeLoopSegment = normalizeLoopSegment;
globalThis.isValidLoopSegment = isValidLoopSegment;
globalThis.isSameLoopSegment = isSameLoopSegment;
globalThis.insertLoopSegment = insertLoopSegment;
globalThis.updateLoopSegmentBound = updateLoopSegmentBound;
globalThis.removeLoopSegment = removeLoopSegment;
globalThis.advanceLoop = advanceLoop;
globalThis.loopEditAnchor = loopEditAnchor;
globalThis.needsOverlayRemount = needsOverlayRemount;
globalThis.shouldRebindVideo = shouldRebindVideo;
globalThis.isLoopBookmark = isLoopBookmark;
globalThis.loopEndForBookmark = loopEndForBookmark;
globalThis.loopSegmentsFromBookmarks = loopSegmentsFromBookmarks;
globalThis.sanitizeLoopName = sanitizeLoopName;
globalThis.formatLoopClock = formatLoopClock;
globalThis.formatLoopRange = formatLoopRange;
globalThis.buildLoopBookmark = buildLoopBookmark;
globalThis.isDuplicateLoop = isDuplicateLoop;
globalThis.LOOP_CONSTANTS = LOOP_CONSTANTS;
